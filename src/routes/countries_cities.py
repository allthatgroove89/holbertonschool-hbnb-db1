...
@country_city_manager_blueprint.route('/', methods=['GET'])
def get_countries():
    countries = Country.query.all()
    return jsonify([countries.to_dict() for countries in countries]), 200


@country_city_manager_blueprint.route('/<country_code>', methods=['GET'])
...
@country_city_manager_blueprint.route('/<country_code>/cities', methods=['GET'])
...
def create_city():
    ...
    if not request.json or 'name' not in request.json \
            or 'country_code' not in request.json:
